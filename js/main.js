const spots = [
  ...
  document.addEventListener("keydown", function (e) {
    ...
    if (e.keyCode == 39) {
        if (currentIndex >= 0){
            ...
            const nextIndex = $('.playerWrapper').length > currentIndex+1 ? currentIndex+1 : 0;
    ...
    if (e.keyCode == 37) {
        if (currentIndex >= 0){
            $('.playerWrapper.focused').removeClass('focused');
            const nextIndex = currentIndex - 1  < 0 ? $('.playerWrapper').length - 1 : currentIndex - 1;
            $($('.playerWrapper')[nextIndex]).addClass('focused');
        }
    }
  });
  ...
  const updateMode = (newMode) => {
    $("#modeSelector > a").removeClass("active");
    $("#modeSelector > a[data-mode="+newMode+"]").addClass("active");
